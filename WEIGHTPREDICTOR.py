...
input_data = pd.DataFrame([[g, h, a]], columns=['Gender', 'Height', 'Age'])
...
with col1:
    st.metric(label='Predicted Weight (kg)', value=f"{w1} kg")
with col2:
    st.metric(label='Predicted Weight (lbs)', value=f"{w2} lbs")


st.success("The weight of a %d year old %s of %.2f m is most likely going to be %.2f kg "%(a,gender,h,p[0]))
st.caption("Made by Group'7'")
